...
st.set_page_config(page_title="Donaldson-Thomas Invariants", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Donaldson-Thomas Invariants</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Calabi-Yau moduli space | Stable configurations | Topological market structure count</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 DT Invariants")
...
st.sidebar.markdown(f"**Partitions:** {config.N_PARTITIONS} | **Stability threshold:** {config.STABILITY_THRESHOLD}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'dt_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>DT invariant: {etf['dt_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized DT Invariant"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized DT Invariant", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Donaldson-Thomas Invariant (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Donaldson-Thomas invariants** count stable configurations (instantons) in a Calabi-Yau moduli space.
        - The moduli space is constructed from the market's correlation structure and macro states.
        - Stable configurations are clusters with sufficient "mass" in the moduli space.
        - The DT invariant is the **number of stable clusters** – a topological count.
        - Higher DT invariant → more coherent market structures → potential alpha.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the DT invariants.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Donaldson-Thomas Invariants | Calabi-Yau moduli space topology")
